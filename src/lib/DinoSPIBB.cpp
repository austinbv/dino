...
//   Bit 0..1  = SPI mode
//   Bit 2..5  = ** unused **
//   Bit 6     = Read bit order:  MSBFIRST(1) or LSBFIRST(0)
//   Bit 7     = Write bit order: MSBFIRST(1) or LSBFIRST(0)
...
void Dino::spiBBtransfer( uint8_t settings, uint8_t select, uint8_t clock, uint8_t input, uint8_t output,
  ...
  // Bit orders are stored in the highest 2 bits of settings.
  uint8_t rBitOrder = (settings & 0b01000000) >> 6;
  uint8_t wBitOrder = (settings & 0b10000000) >> 7;
  ...
  for (byte i = 0;  (i < rLength || i < wLength);  i++) {
    ...
    if (i < wLength) {
      b = spiBBtransferByte(select, clock, input, output, mode, rBitOrder, wBitOrder, data[i]);
    } else {
      b = spiBBtransferByte(select, clock, input, output, mode, rBitOrder, wBitOrder, 0x00);
    }
...
//
byte Dino::spiBBtransferByte(uint8_t select, uint8_t clock, uint8_t input, uint8_t output, uint8_t mode, uint8_t rBitOrder, uint8_t wBitOrder, byte data) {
  // Byte to return
  byte b = 0x00;
  
  // Indices to keep track of which bits out of the 8 are being read and written.
  uint8_t rBit;
  uint8_t wBit;
  ...
  for (int i=0; i<8; i++) {
    ...
    if (wBitOrder == 0) { wBit = i; } else { wBit = 7 - i; }
    if (rBitOrder == 0) { rBit = i; } else { rBit = 7 - i; }

    // SPI MODE 0
    if (mode == 0){
      if (output < 255) digitalWrite(output, bitRead(data, wBit));
      digitalWrite(clock, HIGH);
      if (input < 255) bitWrite(b, rBit, digitalRead(input));
