module Dino
  class Board
    ...
    def uart_start(index, baud, listening=true)
      ...
      self.write Message.encode command:     13,
                                pin:         config,
    ...
    def uart_stop(index)
      raise ArgumentError, "given UART: #{index} out of range. Only 1..3 supported" if (index < 1 || index > 3)
      self.write Message.encode(command: 13, pin: index)
    end
    ...
    def uart_write(index, data)
      ...
      self.write Message.encode(command: 14, pin: index, value: data.length, aux_message: data)
    end
  end
end
