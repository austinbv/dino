...
class InputRegisterTest < Minitest::Test
  ...
  def button
    @button ||= Dino::Components::Button.new(board: part, pin: 0)
  end
  ...
  def test_state_setup
    new_part = Dino::Components::Register::ShiftIn.new(options.merge(bytes: 3))
  ...
  def test_listener_proxy
    ...
    part.stub(:listen, mock) do
      ...
      button1 = Dino::Components::Button.new(board: part, pin: 1)
    end
    mock.verify
