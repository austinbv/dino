  ...
  module Components
    class ShiftRegisterOut
      ...
      def write_bytes(*bytes)
        ...
        # [latch pin, (unused byte), number of data bytes, data byte 1, data byte 2...]
        #
        aux = [latch.pin, 0, aux.count].concat(aux)

        # Pack into literal format for aux msg compatbility then send.
        aux = aux.pack('C*')
        board.write Dino::Message.encode(command: 22, pin: data.pin, value: clock.pin, aux_message: aux)
      end
