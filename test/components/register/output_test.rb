...
class OutputRegisterTest < Minitest::Test
  ...
  def led
    @led ||= Dino::Components::Led.new(board: part, pin: 0)
  end
  ...
  def test_state_setup
    new_part = Dino::Components::Register::ShiftOut.new(options.merge(bytes: 3))
  ...
  def test_write_buffering_control
    new_part = Dino::Components::Register::ShiftOut.new(options.merge(bytes: 3, buffer_writes: false, write_delay: 0.5))
